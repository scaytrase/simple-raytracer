...
rtScene::rtScene() {
    ...
    for (int x = 0; x < 6; ++x)
        for (int y = 0; y < 6; ++y)
            for (int z = 0; z < 6; ++z) {
                rtSphereObject *sphere1 = new rtSphereObject(vertex3f(x - 3, y - 3, z + 0.5f) * 20, 10);
                rtMaterial *material1 = new rtMaterial(QString("PURPLE"), new rtSolidColorTexture(colorPurple));
                material1->setReflection_color(new rtSolidColorTexture(colorLightGray));
                material1->setIoR(new rtSolidColorTexture(colorWhite));
                material1->setIoR_multiplier(1.3f);
                material1->setRefraction_color(new rtSolidColorTexture(colorWhite));
                material1->setReflection_flag(true);
                material1->setRefraction_flag(true);
...
Color3 rtScene::castRay(rayf ray, int level, const bool preview_flag = false) const {
    ...
    vertex3f preintpoint = ray.at(t - 1e-3f);
    vertex3f normal = objectList[id]->material()->remapNormal(objectList[id]->normal(preintpoint));
    normal.normalize();

    if (preview_flag)
        return Color3(normal.x() / 2 + 0.5, normal.y() / 2 + 0.5, normal.z() / 2 + 0.5);

    float IoR =
            objectList[id]->material()->refractionEnabled() ?
                    objectList[id]->material()->getIoR()->getColorAt(intpoint, objectList[id]->getPosition()).b *
                            objectList[id]->material()->getIoR_multiplier()
                    :
                    1.0f;


    float cosT = -(ray.direction * normal);
    float sin2T = (IoR * IoR) * (1 - cosT * cosT);
    ...
    return objectList[id]->material()->combineColor(ambientColor, reflectionColor, refraction_color);
