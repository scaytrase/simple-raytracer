...
float rtbbox::getX(char X, int p) const {
  ...
  case 2: switch (X) {
    case 'x': return point.x() + size.x();
    case 'y': return point.y() + size.y();
    case 'z': return point.z() + size.z();
    }
...
rtbbox * rtbbox::cut(float P, char X, int p) const {
  switch (p) {
  case 1: switch (X) {
    case 'x': return new rtbbox(point, vertex3f(P, size.y(), size.z()));
    case 'y': return new rtbbox(point, vertex3f(size.x(), P, size.z()));
    case 'z': return new rtbbox(point, vertex3f(size.x(), size.y(), P));
    }
  case 2: switch (X) {
    case 'x': return new rtbbox(point + xDirection*P, size - xDirection*P);
    case 'y': return new rtbbox(point + yDirection*P, size - yDirection*P);
    case 'z': return new rtbbox(point + zDirection*P, size - zDirection*P);
    }
   default: return new rtbbox (zero_vertex,zero_vertex);
  }
